...
浏览器打开 `http://127.0.0.1:5173`。PC Web 默认调用后端的 **8000** 端口（`privacyApi.ts`，可用 `VITE_API_BASE_URL` 覆盖）。**注意端口差异**：HarmonyOS 客户端默认 8001、PC Web 默认 8000，同时联调两个端时请让后端同时监听两个端口，或统一端口后配置环境变量。

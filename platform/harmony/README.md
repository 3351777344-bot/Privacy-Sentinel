...
- Privacy 优先通过系统文件选择器从手机文件选图，也可选系统相册；并接入真实检测与脱敏接口。
- Privacy 提供“加载内置演示图”，保证无演示素材时也能稳定跑通 OCR、二维码和隐私检测链路。
...
- 四模块真实检测结果通过 ArkData Preferences 保存在本机，最多保留 100 条；首页展示已存数量、平均分、待复核数和最近记录，并提供历史报告管理页用于筛选、查看完整证据、删除和清空记录。
- 图片脱敏、保存及分享续接会回写历史处置状态；旧版摘要记录会在读取时自动补齐兼容字段。
